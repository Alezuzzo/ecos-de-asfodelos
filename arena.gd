...
# --- NOVA FUNÇÃO EXCLUSIVA PARA O CHEFE ---
func spawnar_inimigo_para_chefe():
	# Esta função NÃO tem a trava "luta_contra_chefe_ativa"
	print("DEBUG: Chefe solicitou spawn de inimigo.")
	
	var inimigo_escolhido = cenas_inimigos.pick_random()
	if not inimigo_escolhido: return
	
	var inimigo = inimigo_escolhido.instantiate()
	# Conecta o sinal de morte, mas a Arena ignora durante a luta do chefe
	inimigo.connect("morreu", _on_inimigo_morreu) 
...
func verificar_sinergias(jogador):
	...
	if tem_todas:
		...
		if $HUD and $HUD.has_method("mostrar_notificacao"):
			$HUD.mostrar_notificacao("Baluarte da alma completo!")
...
func _on_inimigo_morreu():
	# Ignora completamente se a luta do chefe estiver ativa
	if luta_contra_chefe_ativa: return 
...
func _on_jogador_morreu():
	...
	if luta_contra_chefe_ativa: boss_music_player.stop()
	else: music_combate.stop()
