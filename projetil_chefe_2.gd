# projetil_chefe_2.gd (ou o nome do seu script)
extends Area2D

@export var velocidade = 500 # Pode ter velocidade diferente
@export var tempo_de_vida = 3.0
@export var dano = 1
# @export var dano = 1 # Se o dano for diferente
...
func _on_body_entered(body):
	if body.is_in_group("jogador"):
		if body.has_method("sofrer_dano"):
			# Use a variável de dano se tiver uma, senão use 1
			var dano_causado = dano if "dano" in self else 1
			body.sofrer_dano(dano_causado)
		queue_free()

# Função que DEVE existir para o sinal timeout()
